...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pagestat: 'Exibindo de {from} a {to} de {total} registros',
		...
		if (p.searchitems)
			{
				...
				var sopt = "";
				var s
				for ( s = 0; s < p.searchitems.length; s++)
				{
					if(typeof(p.searchitems[s])=="undefined") continue;
					if (p.qtype=='' && p.searchitems[s].isdefault==true)
					{
					p.qtype = p.searchitems[s].name;
					sel = 'selected="selected"';
					} else sel = '';
						sopt += "<option value='" + p.searchitems[s].name + "' " + sel + " >" + p.searchitems[s].display + "&nbsp;&nbsp;</option>";						
				}
				
				if (p.qtype=='') p.qtype = p.searchitems[0].name;
