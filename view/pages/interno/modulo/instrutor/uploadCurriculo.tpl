...
<script>$("#statusInformacao").html("Você está em: Instrutores >> Listar >> Upload de Currículo.");</script>

<input type="text" id="idInstrutor" name="idInstrutor" value="{{$instrutor.idInstrutor}}" style="display: none;" />

<div id="users-contain" class="ui-widget" style="margin:20px auto;">
    <table id="users" class="ui-widget ui-widget-content">
        <thead>
            <tr class="ui-widget-header ">
                <th>Upload de Currículo</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    ...
                    <fieldset style="background-color: whitesmoke;">
                        ...
                        <form enctype="multipart/form-data" id="form" action="{{$BASE_PATH}}interno/modulo/instrutor/uploadCurriculo" method="post">
                            <input type="hidden" name="MAX_FILE_SIZE" value="30000000" />
                            <input type="hidden" id="uploadCurriculoInstrutor_nomeUpload" name="uploadCurriculoInstrutor_nomeUpload" />
                            Arquivo do Currículo: <input name="userfile" type="file" id="userfile" onchange="valida();" />
...
<script type="text/javascript" charset="utf-8">
    ...
    function cadastrar() {
        ...
        if (contador == 0) {
            ...
            //ESTA FUNÇÃO
            $.ajax({
                url: "{{$BASE_PATH}}interno/modulo/instrutor/armazenarCurriculo",
                data: {
                ...
                success: function(data) {
                    ...
                    $('#porcentagem').html('80%');
                    $('#uploadCurriculoInstrutor_nomeUpload').val(respostaDoControlador.message);
