...
<div id="resultado-contain">
    ...
    <table id="resultado">
        ...
        <tbody id="conteudoResultado">
            ...
            <tr>
                <td>{{$tipoLancamentos.idTipoLancamento}}</td>
                <td>{{$tipoLancamentos.nome}}</td>
                <td>{{$tipoLancamentos.descricao}}</td>
                <td>{{$tipoLancamentos.usuarioCriacao}}</td>
                <td>{{$tipoLancamentos.dataCriacao}}</td>
                <td>
                    <button id="{{$tipoLancamentos.idTipoLancamento}}" onclick="javascript:abreEditarTipoLancamento(this);" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="cursor:pointer;" title="Atualizar"><img src="{{$IMG}}../imgs/atualizar.png"></button>
                    <button id="{{$tipoLancamentos.idTipoLancamento}}" onclick="abrirExcluir(this)" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="cursor:pointer;" title="Excluir"><img src="{{$IMG}}../imgs/excluir.png"></button>
                </td>
            </tr>
            {{/foreach}}
        </tbody>
...
<script type="text/javascript" charset="utf-8">
    ...
    function abreEditarTipoLancamento(variavel) {
        $.ajax({
            url: "{{$BASE_PATH}}interno/modulo/financeiro/tipolancamento/buscar",
            data: {
    ...
    function gravarTipoLancamento() {
        ...
        if (contador == 0) {
            ...
            $.ajax({
                url: "{{$BASE_PATH}}interno/modulo/financeiro/tipolancamento/cadastrar",
                data: {
    ...
    function editarTipoLancamento() {
        ...
        if (contador == 0) {
            ...
            $.ajax({
                url: "{{$BASE_PATH}}interno/modulo/financeiro/tipolancamento/editar",
                data: {
    ...
    function confirmarExclusaoTipoLancamento() {
        openlink('{{$BASE_PATH}}interno/modulo/financeiro/tipolancamento/excluir/' + $("#auxDialogsTipoLancamento").val())
        openlink("{{$BASE_PATH}}interno/modulo/financeiro/tipolancamento/listar/");
    }
