...
<div id="resultado-contain">
    ...
    <table id="resultado">
        ...
        <tbody id="conteudoResultado">
            ...
            <tr>
                <td>{{$centroCustos.idCentroCusto}}</td>
                <td>{{$centroCustos.nome}}</td>
                <td>{{$centroCustos.descricao}}</td>
                <td>{{$centroCustos.usuarioCriacao}}</td>
                <td>{{$centroCustos.dataCriacao}}</td>
                <td>
                    <button id="{{$centroCustos.idCentroCusto}}" onclick="javascript:abreEditarCentroCusto(this);" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="cursor:pointer;" title="Atualizar"><img src="{{$IMG}}../imgs/atualizar.png"></button>
                    <button id="{{$centroCustos.idCentroCusto}}" onclick="abrirExcluir(this)" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="cursor:pointer;" title="Excluir"><img src="{{$IMG}}../imgs/excluir.png"></button>
                </td>
            </tr>
            {{/foreach}}
        </tbody>
...
<script type="text/javascript" charset="utf-8">
    ...
    function abreEditarCentroCusto(variavel) {
        $.ajax({
            url: "{{$BASE_PATH}}interno/modulo/financeiro/centrocusto/buscar",
            data: {
    ...
    function gravarCentroCusto() {
        ...
        if (contador == 0) {
            ...
            $.ajax({
                url: "{{$BASE_PATH}}interno/modulo/financeiro/centrocusto/cadastrar",
                data: {
    ...
    function editarCentroCusto() {
        ...
        if (contador == 0) {
            ...
            $.ajax({
                url: "{{$BASE_PATH}}interno/modulo/financeiro/centrocusto/editar",
                data: {
    ...
    function confirmarExclusaoCentroCusto() {
        openlink('{{$BASE_PATH}}interno/modulo/financeiro/centrocusto/excluir/' + $("#auxDialogsCentroCusto").val())
        openlink("{{$BASE_PATH}}interno/modulo/financeiro/centrocusto/listar/");
    }
