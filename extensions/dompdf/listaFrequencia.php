...
$result = mysql_query("
		    ...
		    (
			    ...
			    LEFT JOIN matricula m on m.idTurma = t.idTurma
	    		)
			left join aluno a on a.idAluno = m.idAluno
		    )
		    left join empresa e on e.idEmpresa = a.vinculo 
                WHERE 
...
$db = mysql_connect("localhost", "root", "");
mysql_select_db("sistemas_segura", $db);
$result = mysql_query("SELECT m.idMatricula, a.nome, e.nomeFantasia as vinculo, a.cpf FROM (matricula m LEFT JOIN aluno a on m.idAluno = a.idAluno) left join empresa e on a.vinculo = e.idEmpresa WHERE m.idTurma = $idTurma and a.status != 'Excluido' and m.status = 'Matriculado'ORDER BY a.nome", $db);
...
while ($row = mysql_fetch_array($result)) {
    ...
    if ($contador <= 20) {
        $aux = $aux . "<tr><td>" . $contador . "</td><td>" . $row["nome"] . "</td><td>" . $row["vinculo"] . "</td><td></td></tr>";
    } else if ($contador > 20 && $contador <= 40) {
        $aux2 = $aux2 . "<tr><td>" . $contador . "</td><td>" . $row["nome"] . "</td><td>" . $row["vinculo"] . "</td><td></td></tr>";
    } else if ($contador > 40 && $contador <= 60) {
        $aux3 = $aux3 . "<tr><td>" . $contador . "</td><td>" . $row["nome"] . "</td><td>" . $row["vinculo"] . "</td><td></td></tr>";
    } else if ($contador > 60 && $contador <= 80) {
        $aux4 = $aux4 . "<tr><td>" . $contador . "</td><td>" . $row["nome"] . "</td><td>" . $row["vinculo"] . "</td><td></td></tr>";
    } else if ($contador > 80 && $contador <= 100) {
        $aux5 = $aux5 . "<tr><td>" . $contador . "</td><td>" . $row["nome"] . "</td><td>" . $row["vinculo"] . "</td><td></td></tr>";
    }
}
mysql_free_result($result);
mysql_close($db);
            ...
            <div id="cabecalho">
                ...
                <div id="cabecalhoDetalhesDireito">
                    <table style="font-size: 16px;" border = 1>
                        <tr>
                            <td rowspan="2" style="font-size: 22px; text-align: center;"> LISTA DE FREQUÊNCIA </td>
                            <td> FORM-SEGURA-017 </td>
                        </tr>
                        <tr>
                            <td> Revisão: 02 </td>
                        </tr>
