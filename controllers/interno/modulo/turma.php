...
//Classe de controle onde o próprio nome da função da mostra que ação realiza
    ...
    function verificaPagamento() {
        ...
        if ($consulta == []) {
            $this->pagamentoAntigo($idMatricula);
        } else {
            $this->pagamentoNovo($FormaPagamento, $ValorBaixado, $idMatricula, $matricula, $consulta);
        }
    }

    function pagamentoAntigo($idMatricula) {
        //Inicia a sessão
        $this->init_session();
    ...
    function pagamentoNovo($FormaPagamento, $ValorBaixado, $idMatricula, $matricula, $consulta) {
        $this->init_session();
