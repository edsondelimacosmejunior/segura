 ...
 * @package Controllers
 */
Class Controller_Contasreceber extends Proto_Controller {
    ...
    function mostrar() {
        ...
        //Abre a página
        $this->show("pages/interno/modulo/financeiro/relatorios/contasReceber.tpl");
    }
    ...
    function gerar() {
        ...
        $lancamentos = Doctrine_Query::create()
                ...
                ->where("l.status like 'Pendente' and l.pagarReceber = 1 and l.dataVencimento >= '$dataInicial' and l.dataVencimento <= '$dataFinal' $complementoSql")
                ->execute()
                ->toArray();

        $valorTotal = 0;
        for ($i = 0; $i < sizeof($lancamentos); $i++) {
            $valorTotal += $lancamentos[$i]["valorOriginal"];
