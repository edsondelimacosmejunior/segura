 ...
 * @package Controllers
 */
Class Controller_Fluxocaixaprevisto extends Proto_Controller {
    ...
    function mostrar() {
        ...
        $lancamentos = Doctrine_Query::create()
                ...
                ->where("l.status not like 'Excluido' and l.dataVencimento >= '" . $ano . "-" . $mes . "-01' and l.dataVencimento <= '" . $ano . "-" . $mes . "-" . $dias_do_mes . "'")
        ...
        for ($j = 0; $j < count($lancamentos); $j++) {
            $pieces = explode("-", $lancamentos[$j]["dataVencimento"]);

            for ($i = 0; $i < count($retorno); $i++) {

                if ($retorno[$i]["DIA"] == $pieces[2]) {
                    if ($lancamentos[$j]["pagarReceber"] == 0) {
                        $retorno[$i]["DESPESAS"] += $lancamentos[$j]["valorLiquido"];
                        $totalDespesas += $lancamentos[$j]["valorLiquido"];
                    } else if ($lancamentos[$j]["pagarReceber"] == 1) {
                        $retorno[$i]["RECEITAS"] += $lancamentos[$j]["valorLiquido"];
                        $totalReceitas += $lancamentos[$j]["valorLiquido"];
                    }
                }
            }
        }
        ...
        $tiposLancamentosDespesas = Doctrine_Query::create()
                ...
                ->where("t.status like 'Ativo' and l.pagarReceber = 0 and l.status not like 'Excluido' and l.dataVencimento >= '" . $ano . "-" . $mes . "-01' and l.dataVencimento <= '" . $ano . "-" . $mes . "-" . $dias_do_mes . "'")
                ->orderBy("t.nome")
                ->execute()
                ->toArray();
        ...
        $tiposLancamentosReceitas = Doctrine_Query::create()
                ...
                ->from("Tipolancamento t")
                ->leftJoin("t.Lancamentofinanceiro l")
                ->where("t.status like 'Ativo' and l.pagarReceber = 1 and l.status not like 'Excluido' and l.dataVencimento >= '" . $ano . "-" . $mes . "-01' and l.dataVencimento <= '" . $ano . "-" . $mes . "-" . $dias_do_mes . "'")
                ->orderBy("t.nome")
                ->execute()
                ->toArray();
        ...
        //Abre a página
        $this->show("pages/interno/modulo/financeiro/relatorios/fluxoCaixaPrevisto.tpl");
    }
    ...
    function getReceitas() {
        ...
        $lancamentos = Doctrine_Query::create()
                ->select("l.idLancamentoFinanceiro, "
                        . "l.valorLiquido")
                ->from("Lancamentofinanceiro l")
                ->where("l.status not like 'Excluido' and l.pagarReceber = 1 and l.dataVencimento = '" . $ano . "-" . $mes . "-" . $dia . "' and l.idTipoLancamento = $idTipoLancamento")
                ->execute()
                ->toArray();
        
        $valorTotal = 0;
        for ($i = 0; $i < count($lancamentos); $i++) {
            $valorTotal += $lancamentos[$i]["valorLiquido"];
        }
    ...
    function getDespesas() {
        ...
        $lancamentos = Doctrine_Query::create()
                ->select("l.idLancamentoFinanceiro, "
                        . "l.valorLiquido")
                ->from("Lancamentofinanceiro l")
                ->where("l.status not like 'Excluido' and l.pagarReceber = 0 and l.dataVencimento = '" . $ano . "-" . $mes . "-" . $dia . "' and l.idTipoLancamento = $idTipoLancamento")
                ->execute()
                ->toArray();
    ...
    function getDespesasTotal() {
        ...
        $lancamentos = Doctrine_Query::create()
                ->select("l.idLancamentoFinanceiro, "
                        . "l.valorLiquido")
                ->from("Lancamentofinanceiro l")
                ->where("l.status not like 'Excluido' and l.pagarReceber = 0 and l.dataVencimento >= '" . $ano . "-" . $mes . "-01' and l.dataVencimento <= '" . $ano . "-" . $mes . "-" . $dias_do_mes . "' and l.idTipoLancamento = $idTipoLancamento")
                ->execute()
                ->toArray();
    ...
    function getReceitasTotal() {
        ...
        $lancamentos = Doctrine_Query::create()
                ->select("l.idLancamentoFinanceiro, "
                        . "l.valorLiquido")
                ->from("Lancamentofinanceiro l")
                ->where("l.status not like 'Excluido' and l.pagarReceber = 1 and l.dataVencimento >= '" . $ano . "-" . $mes . "-01' and l.dataVencimento <= '" . $ano . "-" . $mes . "-" . $dias_do_mes . "' and l.idTipoLancamento = $idTipoLancamento")
                ->execute()
                ->toArray();
